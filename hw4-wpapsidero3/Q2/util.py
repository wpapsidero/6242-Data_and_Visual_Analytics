...
def entropy(class_y):
    ...
    y_array = np.array(class_y)
    x_unique, yc = np.unique(y_array, return_counts=True)
...
def information_gain(previous_y, current_y):
    ...
    ent = entropy(previous_y)
    long = len(previous_y)
    curry = current_y[0]
    curry1 = current_y[1]
    l_ent = float(len(curry))/float(long)*entropy(curry)
    r_ent = float(len(curry))/float(long)*entropy(curry1)
    info_gain = ent - l_ent - r_ent
    return info_gain
