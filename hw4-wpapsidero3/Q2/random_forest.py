...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        randos = [random.randrange(n) for index in range(n)]
        labels = [XX[index][-1] for index in randos] 
        samples = [XX[index][:-1] for index in randos] 
    ...
    def fitting(self):
        ...
        for index in range(self.num_trees):
            sol = self.decision_trees[index]
            x = self.bootstraps_datasets[index]
            y = self.bootstraps_labels[index]
            sol.learn(x, y)
