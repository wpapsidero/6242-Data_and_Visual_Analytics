...
#############################################
            node_dict={}
            node_dict.update( {"source": source, "target": target})
            source_list= []
            source_list.append(source)
            target_list= [target]
            unique_a = np.empty
            unique_a= np.unique(source)    
            unique_b= np.unique(target)    
            print(node_dict)
            #combo=np.concatenate(unique_a,unique_b)      
        #for i in combo:
            #source_count= Counter(source_list)
            #target_count= Counter(target_list)
            #node_degree= source_count + target_count 
       
       
            def max_node_id(node_degree):
                v=list(node_degree.values())
                k=list(node_degree.keys())
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='sample command:  py pagerank.py -i 5 -d 0.85 simplified_pagerank soc-wiki-elec.edges')
